...
class SherpaTtsEngine private constructor(
    ...
    override fun synthesize(
        ...
        try {
            // sherpa streams via callback; emit each chunk as it's produced.
            engine.generateWithCallback(text = text, sid = sid, speed = rate) { samples ->
                if (stopRequested) return@generateWithCallback 0
                onAudioChunk(AudioHelper.floatToPcm16(samples))
